...
class TreeNumberObject(object):
	def __init__(self, id=None, tree_number=None):
		self.id = id
...
class TreeNumberResource(Resource):
	...
	tree_number = fields.CharField(attribute='tree_number')
	...
	def get_object_list(self, bundle, **kwargs):
		if 'tree_id' not in bundle.request.GET:
	...
	def dehydrate(self, bundle):
		...
		if bundle.obj.tree_number == "A":
			...
			if is_descriptor:
				...
				pharmacological_action_list = list(PharmacologicalActionList.objects.filter(
					identifier_id=identifier_id,
					language_code=lang_code).values('term_string'))
				for action in pharmacological_action_list:
					action['pharmacological_action'] = action.pop('term_string')
					action['attr'] = {'lang': lang}
