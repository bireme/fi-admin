...
from models import Serial, SerialType
...
class SerialIndex(indexes.SearchIndex, indexes.Indexable):
    ...
    serial_collection = indexes.CharField(model_attr='serial_collection', null=True)
    ...
    serial_type = indexes.MultiValueField()
    serial_type_filter = indexes.MultiValueField()
    ...
    def prepare_serial_type(self, obj):
        return [ serial_type.acronym for serial_type in SerialType.objects.filter(media=obj.id) ]

    def prepare_serial_type_filter(self, obj):
        return [ "|".join( serial_type.get_translations() ) for serial_type in SerialType.objects.filter(media=obj.id) ]
